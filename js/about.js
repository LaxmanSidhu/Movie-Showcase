...
// -----------------------------
// CSV parser (same approach as movies.js)
// -----------------------------
...
function safeMovieCountFromRows(rows) {
  // Movies are in first column (movie name) on your sheets
  // Count only rows where movie name exists.
  let count = 0;
...
function ensureCards() {
  ...
  Object.entries(SHEETS)
    .filter(([key]) => key !== "fav_movie" && key !== "watchlist")
    .forEach(([key, sheet]) => {
...
async function refreshCounts() {
  ...
  const entries = Object.entries(SHEETS).filter(
    ([key]) => key !== "fav_movie" && key !== "watchlist"
  );
...
window.setInterval(refreshCounts, 60 * 1000);
